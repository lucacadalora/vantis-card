    ...
    sections: [
      {
        id: "catalog", title: "The catalog", body: `
          ...
          ${note("Reasoning on Kimi K3", `Kimi K3 always reasons. A request on it that carries ${inline('thinking: {"type": "disabled"}')}, ${inline("enable_thinking: false")} or ${inline('reasoning_effort: "none"')} is refused with ${inline("400 reasoning_always_on")} rather than billed for a pass it asked not to have; ${inline("reasoning_effort")} ${inline("low")} / ${inline("high")} / ${inline("max")} (the vendor's ladder, default max) passes through where the route supports it. Reasoning tokens bill as output; prompt-cache reads bill at the published cache-hit price; non-streamed responses return the pass in ${inline("reasoning_content")}, streamed responses carry the answer only.`)}
