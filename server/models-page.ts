...
<section class="sec sec--tight">
  <div class="wrap">
    ...
    ${table(open)}
    <p class="mnote">The default model is what you get when the <code>model</code> field is omitted, and the only route with a failover behind it. <strong>Two tiers, one checkpoint:</strong> the fast tier is the same DeepSeek V4 Flash 0731 build on high-throughput serving (up to 400 tok/s), priced separately at twice the standard rate with prompt-cache reads billed at its cached price. It is also the zero-data-retention route: a call with <code>"zdr": true</code> on either id is served there and billed at the fast rate, and attests itself with <code>X-Vantis-ZDR: honored</code>. Every other id is pinned to a single gateway: if that route is down the call returns an honest 503 rather than quietly answering with a different model or tier. The DeepSeek tiers are <strong>text only</strong> &mdash; send them an image and the gateway refuses with <code>image_input_unsupported</code> rather than letting a model answer about a picture it never received. <strong>Kimi K3</strong> takes image input, runs with reasoning always on (a request that asks to switch it off is refused with <code>reasoning_always_on</code> rather than billed for the pass; <code>reasoning_effort</code> low / high / max passes through), bills prompt-cache reads at its published cache-hit price, and has no zero-data-retention route.</p>
  </div>
</section>
  ...
  <div class="wrap">
    ...
    <div class="finegrid">
      <div class="fine-i">
        ...
        <p>Every rate here is the model vendor&rsquo;s own first-party list price &mdash; DeepSeek&rsquo;s for the DeepSeek line, Moonshot AI&rsquo;s for Kimi K3 (including its published cache-hit input price), OpenAI&rsquo;s for the GPT family. A model we can serve but cannot price does not appear here at all, and the rail has never carried an invented number, because the cost basis is what settles as an on-chain $VANTIS burn.</p>
      </div>
      <div class="fine-i">
